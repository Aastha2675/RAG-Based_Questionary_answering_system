...
* **Multi-Query Retrieval:** Uses the LLM to paraphrase the user query into 3 variations, overcoming keyword-matching limitations in financial terminology.
* **Re-ranking (Cross-Encoder):** Uses `ms-marco-MiniLM-L-6-v2` to re-evaluate the top 5 retrieved chunks, selecting only the top 3 most relevant context snippets for the LLM.
